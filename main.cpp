...
using namespace std;
// Question 1
string passGen(string words[], int number_words) {
...
//Question 3
bool search_letter(string source, char keyValue) {
    for (int i = 0; i < source.length(); i++) {
        if (source[i] == keyValue) {
            return true;
        }
    }
    return false;
...
int main() {
    ...
    string new_password = rep_with_upper(initialPassword);
    cout << "Password after rep_with_upper: " << new_password << endl;

    string finalPassword = swap_letters(new_password);
    ...
    if (search_letter(finalPassword, firstletterofname)) {
        cout << "Key letter is found " << endl;
    } else {
        cout << "Key letter is not found " << endl;
    }
